...
void SaveDescription()
{
	...
	case 1: //UA
	{
		...
		std::cout << " Буде збережена тiльки статистика гравця, досягнення та поточний рiвень";
		break;
	}
	case 2: //RU
	{
		printColorText(consoleHandle, "  Внимание!", Yellow);
		std::cout << " Игра сохраняет только статистику игрока, достижения и текущий\n  уровень";
		break;
	}
	case 3: //ENG
	{
...
void WrongKeyWarning(int key)
{
	...
	{
		printColorText(consoleHandle, "  Увага!", Yellow);
		std::cout << " Клавiша " << (char)key << " не може перезапускати гру";
		break;
	}
	case 2: //RU
	{
		printColorText(consoleHandle, "  Внимание!", Yellow);
		std::cout << " Клавиша " << (char)key << " не может быть назначена для рестарта игры";
		break;
	}
	case 3: //ENG
	{
		printColorText(consoleHandle, "  Attention!", Yellow);
		std::cout << " Key " << (char)key << " cannot be a restart key";
		break;
	}
	}
}